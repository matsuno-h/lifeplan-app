...
### セキュリティ設定（要Supabaseダッシュボード設定）

本番環境では、以下のSupabase Auth設定を推奨します。Supabaseダッシュボードから手動で設定してください。

#### 1. 漏洩パスワード保護の有効化
**設定場所**: Authentication > Providers > Email > Advanced Settings

- **Enable Leaked Password Protection**: ONにする
- この設定により、HaveIBeenPwned.orgのデータベースと照合し、既に漏洩したパスワードの使用を防ぎます
- ユーザーのセキュリティを大幅に向上させます

#### 2. データベース接続戦略の最適化
**設定場所**: Database > Connection Pooling > Auth Configuration

- **Connection Strategy**: `固定数` から `パーセンテージベース` に変更
- 推奨設定: `10%` 以上
- この設定により、インスタンスサイズ変更時にAuth接続が自動的にスケールします
- 現在の固定10接続では、インスタンス拡張時にパフォーマンス向上が制限されます
...
### 2026-01-15
- **セキュリティ最適化**: 未使用のデータベースインデックスを削除し、パフォーマンスを改善しました
- **資産ポートフォリオの改善**: 「資産ポートフォリオ」を「金融資産ポートフォリオ」に名称変更し、不動産を円グラフの集計から除外しました
- **セキュリティ設定ドキュメント追加**: Supabase Authの推奨セキュリティ設定をREADMEに追加しました
